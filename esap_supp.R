...
source('../esap_pdf.R')
esap_supp <- function(time, status){
      ...
      #x.min <- round(min(z),3)#z is log transformed so its value is rounded up to third decimal
      x.min <- min(z)
      sh.mi <- saddle.me(x.min)
      
      #while(sh.mi<phi-.1 & (exp(x.min)-1)>0){#for small values of time, s is negative
      while(sh.mi<phi-.1 & x.min>0){
            #it will decrease by .01 but the logic in the while fail after subtracting for a good one
            x.min <- x.min-.01
            sh.mi <- saddle.me(x.min)
            if(round(M.mgf(sh.mi),1)<=0|round(K2.exp(sh.mi),1)<=0){break}
      }  
      ...
      #x.max=round(max(z),3)# or emp.mean?
      x.max <- max(z)
      pdf <- esap_pdf(exp(x.max)-1,time,status)
      #pdf <- esap_pdf(x.max,time,status)
      sh.m <- saddle.me(x.max)
      
      if(delta[n]==0){
            while(sh.m < (phi-.1) & round(pdf,2)>0) {
      ...
      #     sh.m <- sh.m
      #     v <- 1
      #     while(sh.m[v] < (phi-.1) & round(pdf,3)>0 & ((sh.m[v]-sh.m[v-1])<.001)) {
      #         x.max <- x.max+.01#it will increase by .01 but the logic in the while fail thereafter
      #         #pdf <- esap_pdf(exp(x.max)-1,time,status)
      #         pdf <- esap_pdf(x.max,time,status)
      #         sh.m[v] <- saddle.me(x.max)
      #         v <- v+1
      #     }  
      # }else{
      #     #x.max=z[n]
      ...
      minSp <- round(x.min+.01,3)
      #maxSp <- round(exp(x.max-.01)-1,3)
      maxSp <- round(x.max-.01,3)
